...
public final class CallEditor extends ExprEditor {
   ...
   private static String CODE3 = CODE_STACK + CODE_REMOVE + CODE_INDEX + CODE_SEP1 + CODE_THIS + CODE_END;
   private static String CODE4 = CODE_STACK + CODE_ADD_VAL + CODE_INDEX + CODE_SEP1 + CODE_THIS + CODE_END;
      ...
      public void edit (final MethodCall call) throws CannotCompileException {
         if (call.getMethodName().equals("set")) {
            ...
            call.replace("{ $_ = $proceed($$); "+ CODE3 +"}"); //after operation
            //call.replace("{ "+CODE3+" $_ = $proceed($$); }"); //before operation
         } else if  (call.getMethodName().equals("add")) {
            System.out.println ("Found call to 'add'");
            call.replace("{ $_ = $proceed($$); "+ CODE4 +"}"); //after operation
            //call.replace("{ "+CODE4+" $_ = $proceed($$); }"); //before operation
         }
      }
}
